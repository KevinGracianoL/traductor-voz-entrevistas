...
def test_sintetizar_exaggeration_frontera(tmp_path: Path) -> None:
    ...
    mock_cls = _inject_chatterbox()
    try:
        sintetizar("hello", str(ref), exaggeration=0.0)
        sintetizar("hello", str(ref), exaggeration=1.0)
        assert mock_cls.from_pretrained.return_value.generate.call_count == 2
    finally:
        sys.modules.pop("chatterbox", None)
        sys.modules.pop("chatterbox.mtl_tts", None)
    ...
    with pytest.raises(ValueError) as excinfo:
        sintetizar("hello", str(ref), exaggeration=1.1)
