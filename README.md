...
- [x] **Fase 2f — Go/no-go del motor TTS** — **XTTS-v2 ACEPTADO por los gates medidos** del [ADR-014](docs/ADR-014-gates-aceptacion-tts.md): pipeline end-to-end 1665.4 ms < 2000 ms con 100 % atribuido (ASR 780.7 + traducción 177.6 + TTFA 721.0 + ruteo primer-sample 221.2; tres métricas mal definidas corregidas con evidencia: 400 ms literal, drenado de ruteo, drenado en la escritura); aprobación final condicionada a los gates de sesión (ADR-019). Candidato B (Supertonic 3 + OpenVoice V2) **rechazado** por TTFA arquitectural (7885.8 ms)
