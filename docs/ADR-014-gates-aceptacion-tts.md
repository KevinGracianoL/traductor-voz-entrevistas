...
**Veredicto: XTTS-v2 ACEPTADO por los gates MEDIDOS del ADR-014** (pipeline p50 1107.1 ms / p95 1158.2 ms, rango 1059.9-1171.5 en 19 corridas; 0 de 19 sobre 2000 ms; margen ~830-940 ms). **Estabilidad n=20 corridas:** desglose estable corrida a corrida (ASR encadenado 176-187 ms, TTS 676-779 ms, entrega 15-20 ms). UNA de las 20 corridas fallo en la etapa de TRADUCCION (argos intento descargar el modelo spacy `mwt` on-demand y la red fallo): hallazgo real - la etapa de traduccion tiene una descarga bajo demanda sensible a red; el flujo debe precargar ese modelo. La aprobacion FINAL queda condicionada a los gates de sesion (ADR-019): OOM, crecimiento de memoria, artefactos, voz reconocible en A/B (lo firma el usuario escuchando) y endurance de 90 minutos - pendiente no es aprobado. Margen saludable (829-940 ms sobre el peor caso medido): la optimizacion del ASR queda descartada de momento (ya no es el cuello).