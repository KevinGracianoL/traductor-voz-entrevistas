...
- **Estado:** Propuesto (2026-09-08) — se aprueba cuando un backend real (worker XTTS) lo implemente sin forzar cambios de contrato.
- **Contexto:** Fase 2 necesita TTS offline con clonación de voz (timbre de Kevin, ES→EN) que conviva con Whisper int8 (~1 GB) en una GTX 1650 Ti de 4 GB, dentro del presupuesto ADR-003. Chatterbox fue rechazado con mediciones (ADR-010) y XTTS-v2 quedó descartado antes (VRAM + licencia no-comercial). El pipeline (traducción → TTS → audio virtual) no debe depender del proveedor final mientras se evalúa el candidato.
- **Decisión propuesta:** definir contratos neutrales en `src/traductor/tts/` y escribir los PRs siguientes (benchmark ASR bidireccional, worker XTTS, gates de latencia) contra ellos:
  ...
  - **`AudioResult` / `Salud`** (frozen dataclasses): audio (bytes + formato + duración opcional) y estado del backend (disponible + detalle).
...
- **Consecuencias:**
  ...
  - Si el worker XTTS obliga a cambiar una firma, se cambia aquí con su propio ADR, antes de que haya integraciones que migrar.
