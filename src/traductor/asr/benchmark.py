...
def resumen_asr(resultados: list[ResultadoAsr]) -> dict[str, dict[str, float | None]]:
    """Resumen por `engine|idioma`: n, p50_ms, p95_ms y wer_mean.

    `wer_mean` solo sobre resultados con `referencia` (sin ground truth, None).
    """
    ...
    for clave, rs in agrupado.items():
        wers = [wer(r.referencia, r.frase) for r in rs if r.referencia]
...
def tabla_comparativa(resumen: dict[str, dict[str, float | None]]) -> str:
    ...
    cabecera = "engine          idioma   n   p50_ms   p95_ms     wer"
    if not resumen:
        return cabecera + "\n\n(sin datos)"
    filas = [cabecera]
